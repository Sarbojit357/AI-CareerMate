...
def input_pdf_setup(uploaded_file_bytes):
    """Convert PDF to base64 encoded image using PyMuPDF - CACHED"""
    try:
        pdf_document = fitz.open(stream=uploaded_file_bytes, filetype="pdf")
        first_page = pdf_document[0]
        
        # OPTIMIZED: Changed from Matrix(2, 2) to Matrix(1.5, 1.5)
        pix = first_page.get_pixmap(matrix=fitz.Matrix(1.5, 1.5))
        
        img = Image.frombytes("RGB", [pix.width, pix.height], pix.samples)
        
        # OPTIMIZED: Reduced quality from default to 85
...
def chatbot_response(user_query, job_description, pdf_content_hash):
    """Generate chatbot response using Gemini - CACHED"""
    pdf_content = st.session_state.get('current_pdf_content')
    if not pdf_content:
        return "Error: PDF content not found"
...
@st.cache_data(ttl=3600, show_spinner=False)
def extract_keywords_with_gemini(job_description, pdf_content_hash):
    """Extract keywords using Gemini AI - CACHED"""
    pdf_content = st.session_state.get('current_pdf_content')
    if not pdf_content:
        return "Error: PDF content not found"
...
# OPTIMIZED: Cache resume suggestions
@st.cache_data(ttl=3600, show_spinner=False)
def generate_resume_suggestions(job_description, section):
    """Generate AI suggestions for resume content - CACHED"""
    try:
        model = genai.GenerativeModel("gemini-2.5-flash")
...
def generate_ideal_project(job_description, pdf_content_hash):
    """Generate an ideal project idea based on resume and job description - CACHED"""
    pdf_content = st.session_state.get('current_pdf_content')
    if not pdf_content:
        return "Error: PDF content not found"
...
def main():
    ...
    if uploaded_file is not None:
        ...
        try:
            # Read PDF bytes and create hash for caching
    ...
    with tab1:
        if pdf_content and input_text:
            ...
            with st.spinner("Analyzing your resume against the job description..."):
                ...
                response = get_gemini_response(input_prompt1, pdf_hash, input_text)
    ...
    with tab2:
        if pdf_content and input_text:
            ...
            with st.spinner("Calculating match percentage..."):
                ...
                """
                pdf_hash = st.session_state.get('pdf_hash', '')
                response = get_gemini_response(input_prompt3, pdf_hash, input_text)
        ...
        if pdf_content and input_text:
            ...
            with st.spinner("Extracting and analyzing keywords..."):
                ...
                with col1:
                    ...
                    pdf_hash = st.session_state.get('pdf_hash', '')
                    ai_keywords = extract_keywords_with_gemini(input_text, pdf_hash)
    ...
    with tab4:
        if pdf_content and input_text:
            st.markdown('<div class="section-header"><h3>💬 ATS Resume Expert Chatbot</h3></div>',
                        unsafe_allow_html=True)
        ...
        if pdf_content and input_text:
            ...
            if st.button("🚀 Generate Ideal Project", use_container_width=True):
                with st.spinner(
                    ...
                    pdf_hash = st.session_state.get('pdf_hash', '')
                    project_idea = generate_ideal_project(input_text, pdf_hash)
    ...
    <div style="text-align:center; margin-top:40px; padding:20px; border-top:1px solid #eee; color:#666;">
        <p>ATS Resume Expert - Powered by Gemini AI</p>
